...
(function() {
    ...
    var NavigationConfig = {
        ...
        getPageType: function() {
            ...
            if (pathname.includes('payment.html')) return 'payment';
        ...
        getAuthButtons: function(language, pageType, isLoggedIn, userInfo) {
            ...
            if (isLoggedIn && userInfo) {
                ...
                return [
                    ...
                    {
                        text: isEnglish ? 'PREMIUM' : '高级版',
                        href: this.getPaymentHref(language, pageType),
                        class: 'payment-btn',
                        id: 'payment',
                        type: 'payment'
                    },
        ...
        // Get payment href
        getPaymentHref: function(language, pageType) {
            const isCurvePage = pageType === 'curve';
            const basePath = isCurvePage ? '../' : '';
            const langParam = '?lang=' + language;
            
            return basePath + 'payment.html' + langParam;
        },
        ...
        getLanguageSwitchHref: function(currentLanguage, pageType) {
          ...
          // Removed getUserSettingsHref and all isUserSettingsPage logic
    };
    ...
    var NavigationLoader = {
        ...
        setupEventHandlers: function(language, pageType) {
            ...
            window.goToHomePage = function() {
                ...
                if (isLoginRegisterPage || isCurvePage || isPaymentPage) {
                    const mainPage = 'index.html?lang=' + language;
                    const basePath = isCurvePage ? '../' : '';
            ...
            const paymentButton = document.querySelector('[data-auth-id="payment"]');
            if (paymentButton) {
        ...
        handleLogout: async function(language) {
            try {
                // Use Appwrite logout if available
                ...
                setTimeout(() => {
                    const homePage = 'index.html?lang=' + language;
                    window.location.href = homePage;
                }, 1500);
            } catch (error) {
                console.error('Logout error:', error);
                // Still clear localStorage even if Appwrite logout fails
                localStorage.removeItem('userRegistered');
                localStorage.removeItem('userInfo');
                localStorage.removeItem('authToken');
                localStorage.removeItem('username_token');
    ...
    // Listen for language changes to update navigation
